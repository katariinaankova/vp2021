<?php
   ...
   function read_all_films(){
	  ...
	  while($stmt->fetch()){
		  ...
		  $film_html .= "\n <h3>" .$title_from_db ."</h3> \n";
		  $film_html .= "<ul> \n";
		  $film_html .= "<li>Valmimisaasta: " .$year_from_db;
		  $film_html .= "<li>Kestus: " .$duration_from_db;
		  $film_html .= "<li>Žanr: " .$genre_from_db;
		  $film_html .= "<li>Totja: " .$studio_from_db;
		  $film_html .= "<li>Lavastaja: " .$director_from_db;
		  $film_html .= "</ul> \n";
	  }
	  //sulgen käsu
	  $stmt->close();
   ...
   function store_film($title_input, $year_input, $duration_input, $genre_input, $studio_input, $director_input){
	  ...
	  if($stmt->execute()){
		  ...
		  $succcess = "Salvestamisel tekkis viga: " .$stmt->error;
	  }
